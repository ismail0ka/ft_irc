...
void	cmdMode(Server& srv, Client& c, const Message& m)
{
	...
	{
		std::string	line = ":" + c.getPrefix() + " MODE " + ch->getName() + " " + applied.modes;
...
void	cmdKick(Server& srv, Client& c, const Message& m)
{
	...
	for (std::size_t i = 0; i < victims.size(); ++i)
	{
		...
		ch->broadcast(":" + c.getPrefix() + " KICK " + ch->getName()
...
void	cmdInvite(Server& srv, Client& c, const Message& m)
{
	...
	guest->queue(":" + c.getPrefix() + " INVITE " + guest->getNick() + " :" + ch->getName() + CRLF);
}

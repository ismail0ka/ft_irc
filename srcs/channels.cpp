...
void	cmdJoin(Server& srv, Client& c, const Message& m)
{
	...
	for (std::size_t i = 0; i < names.size(); ++i)
	{
		...
		ch->broadcast(":" + clientPrefix(c) + " JOIN :" + ch->getName() + CRLF, NULL);
		sendTopic(c, *ch);
		sendNames(c, *ch);
	}
}
...
void	cmdPart(Server& srv, Client& c, const Message& m)
{
	...
	for (std::size_t i = 0; i < names.size(); ++i)
	{
		...
		std::string	line = ":" + clientPrefix(c) + " PART " + ch->getName();
...
void	cmdTopic(Server& srv, Client& c, const Message& m)
{
	...
	ch->setTopic(c, topic);
	ch->broadcast(":" + clientPrefix(c) + " TOPIC " + ch->getName() + " :" + topic + CRLF, NULL);
}
